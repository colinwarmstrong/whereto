...
describe 'Favorites Endpoints' do
  context 'GET /api/v1/favorites' do
    it 'returns all favorited cities for a user' do
      ...
      expect(city_1[:favorite]).to eq(true)
      expect(city_1[:rejection]).to eq(false)
      ...
      expect(city_2[:favorite]).to eq(true)
      expect(city_2[:rejection]).to eq(false)
    end
  end
  ...
  context 'POST /api/v1/favorites' do
    it "adds a city to a user's favorites" do
      ...
      expect(new_favorite[:rejection]).to eq(false)
    end
